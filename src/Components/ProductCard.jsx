...
export const ProductCard = ({ image, style, product, getCartItems }) => {
  ...
  return (
    <div
      key={product?.id}
      style={{ ...style }}
      className={`p-3 bg-white rounded-3xl ${isHovered ? "shadow-md" : ""}`}
    ...
    >
      ...
      <figcaption className="product-desc py-1">
        <h2 className="my-2 text-gray-800">
          {product?.product_name ?? "Gold color vintage Geometry"}
        </h2>
        <ul className="my-1 flex text-gray-400">
          <li>xxxxxx</li>
          <li>{product?.sold_count ?? "5000"}+ sold</li>
        </ul>
      ...
      <div className="flex items-center justify-center py-1">
        <Link
          ...
          className={`${
            isHovered ? "visible h-10" : "invisible h-10"
          } rounded-3xl w-full py-2 px-6 font-bold bg-teal-600 text-white`}
